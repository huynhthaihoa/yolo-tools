...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--input", help="Path to the input folder",
                    type=str, default="")
    parser.add_argument("-c", "--calibration", help="Path to the calibration file",
    ...
    with open(args.calibration, "rb") as f:
        ...
        #x, y, w, h = roi
        newcameramtx, _ = cv2.getOptimalNewCameraMatrix(K, D, dim, 1, dim)
        mapx, mapy = cv2.initUndistortRectifyMap(K, D, None, newcameramtx, dim, cv2.CV_16SC2)
        print(mapx)
        print(mapy)
